...
async function byPlaca(req, res) {
  ...
  try {
    const veiculoRows = await sequelize.query('SELECT placa, tipo, fipe_name_marca, fipe_modelo, fipe_name_ano FROM veiculos WHERE placa = ?', { replacements: [placa], type: sequelize.QueryTypes.SELECT });
    const veiculo = veiculoRows[0];
    if (!veiculo) return res.status(404).json({ error: 'Veículo não encontrado' });

    const dateFilter = []; const dateParams = [placa];
    ...
    const multasDateFilter = []; const multasDateParams = [placa];
    ...
    const pgtoDateFilter = []; const pgtoDateParams = [placa];
    ...
    const pgtoDocDateFilter = []; const pgtoDocDateParams = [placa];
    ...
    const [manutencoes, multas, abastecimentos, pagamentosSeguro, pagamentoDocumentos] = await Promise.all([
      sequelize.query(`SELECT id, data, valor, descricao, km, classificacao FROM manutencoes WHERE veiculo_id = ? ${dateClause} ORDER BY data`, { replacements: dateParams, type: sequelize.QueryTypes.SELECT }),
      sequelize.query(`SELECT id, data_ocorrencia, valor, local_ocorrencia, pagamento_realizado, motorista_id FROM multas WHERE veiculo_id = ? ${multasDateClause} ORDER BY data_ocorrencia`, { replacements: multasDateParams, type: sequelize.QueryTypes.SELECT }),
      sequelize.query(`SELECT id, data, valor, quantidade, km, tanque_cheio FROM abastecimentos WHERE veiculo_id = ? ${dateClause} ORDER BY data`, { replacements: dateParams, type: sequelize.QueryTypes.SELECT }),
      sequelize.query(`SELECT id, data_pagamento, valor FROM pagamentos_seguro WHERE veiculo_id = ? ${pgtoDateClause} ORDER BY data_pagamento`, { replacements: pgtoDateParams, type: sequelize.QueryTypes.SELECT }),
      sequelize.query(`SELECT id, data_pagamento, valor, descricao FROM pagamento_documentos WHERE veiculo_id = ? ${pgtoDocDateClause} ORDER BY data_pagamento`, { replacements: pgtoDocDateParams, type: sequelize.QueryTypes.SELECT }),
    ]);
    ...
    if (absSource.length >= 2) {
      for (let i = 1; i < absSource.length; i++) {
        ...
        if (atual.km != null && anterior.km != null && atual.quantidade > 0) {
          ...
          if (kmRodados > 0) consumo.push({ data: atual.data, km_rodados: kmRodados, litros: atual.quantidade, km_por_litro: Math.round((kmRodados / atual.quantidade) * 100) / 100, tanque_cheio: atual.tanque_cheio == 1 });
        }
      }
    }
