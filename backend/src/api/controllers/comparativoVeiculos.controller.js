...
const { calculateKmPerLiter } = require('../utils/helpers');

const VEHICLE_FIELDS = 'placa, numero, tipo, fipe_name_marca, fipe_modelo, fipe_name_ano, cor, km, ano_fab, ano_modelo, cidade, uf, ativo, data_aquisicao, combustivel';
...
async function comparativo(req, res) {
  try {
    ...
    const [veiculo1Rows, veiculo2Rows] = await Promise.all([
      sequelize.query(`SELECT ${VEHICLE_FIELDS} FROM veiculos WHERE placa = ?`, { replacements: [placa1], type: sequelize.QueryTypes.SELECT }),
      sequelize.query(`SELECT ${VEHICLE_FIELDS} FROM veiculos WHERE placa = ?`, { replacements: [placa2], type: sequelize.QueryTypes.SELECT }),
    ]);

    if (!veiculo1Rows[0] || !veiculo2Rows[0]) {
      return res.status(404).json({ error: 'Um dos veículos não foi encontrado' });
    }

    const custo = async (placa, kmAtual) => {
      const [manutencao, combustivel, multas, seguro, higienizacao, ordensServico, kmRows, consumoRows] = await Promise.all([
        ...
        sequelize.query(
          `SELECT MIN(km) as km_min, MAX(km) as km_max
           FROM (
          ...
          { replacements: [placa, placa], type: sequelize.QueryTypes.SELECT }
        ),
        sequelize.query(
          'SELECT a.km, a.quantidade FROM abastecimentos a WHERE a.veiculo_id = ? AND a.km IS NOT NULL AND a.quantidade > 0 AND a.km > 0 ORDER BY a.data DESC LIMIT 5',
          { replacements: [placa], type: sequelize.QueryTypes.SELECT }
        ),
      ]);
      ...
      if (consumoRows.length >= 2) {
        ...
        kml = calculateKmPerLiter(diffKm, litrosEntre) || null;
      }
      ...
      const custoPorKm = kmPercorrido > 0 ? Math.round((totalGeral / kmPercorrido) * 100) / 100 : null;
    ...
    const [c1, c2] = await Promise.all([
      custo(placa1, veiculo1Rows[0].km || 0),
      custo(placa2, veiculo2Rows[0].km || 0),
    ]);
