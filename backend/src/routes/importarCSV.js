...
function buildSynonymIndex(allowedFields) {
  ...
  for (const field of allowedFields) {
    ...
    for (const syn of synonyms) {
      const snorm = normalizeFieldName(syn);
      if (!idx[snorm]) idx[snorm] = field;
    }
  }
  return idx;
}
...
function resolveFieldMapping(csvHeaders, allowedFields) {
  ...
  for (const header of csvHeaders) {
    ...
    if (matched && !used.has(matched)) {
      ...
      used.add(matched);
    } else if (matched && used.has(matched)) {
      mapping[header] = null;
    } else {
      mapping[header] = null;
    }
...
function parseCSV(text) {
  ...
  for (let i = 0; i < cleanText.length; i++) {
    ...
    if (ch === ',' && !inQuotes) {
      fields.push({ value: current.trim(), lineNum });
      current = '';
      continue;
    }
    if ((ch === '\n' || ch === '\r') && !inQuotes) {
...
function createModelCSV(tabela) {
  const examples = {
    veiculos: [
      'ABC1D23,101,Caminhão,Volkswagen,Constellation 19.320,123456789,9BWCXxx,120000,Branco,SP,São Paulo,Caminhão basculante',
      'XYZ9X45,202,Vuc,Mercedes-Benz,Accelo 815,987654321,9BMCXyy,85000,Prata,MG,Belo Horizonte,Entregas urbanas',
    ],
    cnhs: [
      'SP123456789,João Silva,15/03/1990,E,123.456.789-00,15/03/2030,15/03/2020,DETRAN-SP',
      'MG987654321,Maria Souza,20/07/1985,D,987.654.321-00,20/07/2028,20/07/2018,DETRAN-MG',
    ],
    manutencoes: [
      '15/01/2025,2500.00,Troca de óleo e filtros,120000,preventiva,ABC1D23',
      '20/02/2025,850.00,Alinhamento e balanceamento,122000,corretiva,ABC1D23',
    ],
    multas: [
      '10/01/2025,25/02/2025,350.00,Rodovia BR-101 km 210,ABC1D23,123456789',
      '05/03/2025,20/04/2025,195.00,Av. Paulista 1000,XYZ9X45,987654321',
    ],
    abastecimentos: [
      '15/01/2025,200,1200.00,120000,ABC1D23',
      '20/01/2025,180,1080.00,121500,ABC1D23',
    ],
    mecanicas: [
      'Oficina do João,Rua das Ofícinas 100,São Paulo,SP,(11) 99999-0001,joao@oficina.com',
      'Auto Mecânica LTDA,Av. dos Autos 500,Belo Horizonte,MG,(31) 98888-0002,contato@automecanica.com',
    ],
    cidades: [
      'São Paulo,SP',
      'Belo Horizonte,MG',
  ...
  const fields = ALLOWED_TABLES_CSV[tabela];
  const headerLine = fields.join(',');
  const exampleLines = examples[tabela] || [];
  const dataLines = exampleLines.length ? exampleLines : [fields.map(() => '').join(',')];
  return [headerLine, ...dataLines].join('\n');
}

function registerImportarCSVRoutes(app) {
  app.get('/api/importar/csv/modelo/:tabela', (req, res) => {
    const { tabela } = req.params;
    if (!ALLOWED_TABLES_CSV[tabela]) {
  ...
  app.post('/api/importar/csv/preview', requireRole('admin', 'root'), upload.single('file'), async (req, res) => {
    try {
      ...
      if (rows.length < 2) {
        return res.status(400).json({ error: 'CSV vazio ou formato inválido. O CSV precisa ter cabeçalho + pelo menos 1 linha de dados.' });
      }
      ...
      res.json({
        ...
        mapeamento: mapping,
        campos_mapeados: mappedFields,
        campos_ignorados: ignoredFields,
        campos_nao_encontrados: missingFields,
        amostra: sample,
      });
    } catch (error) {
      handleError(res, error, 'importar.csv.preview');
    }
  });
    ...
    try {
      ...
      if (mappedFields.length === 0) {
        return res.status(400).json({
          error: 'Nenhum campo reconhecido. Verifique os cabeçalhos do CSV.',
          campos_esperados: allowedFields,
        });
      }
      ...
      for (const row of dataRows) {
        ...
        try {
          ...
          erros++;
          const linha = rows[dataRows.indexOf(row) + 1]?.lineNum || '?';
      ...
      res.json({
        ...
        campos_mapeados: mappedFields,
        campos_ignorados: ignoredFields,
        ...(detalhesErros.length > 0 && { detalhes_erros: detalhesErros }),
      });
    } catch (error) {
